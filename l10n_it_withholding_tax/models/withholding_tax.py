        ...
        for st in self:
            if st.invoice_id:
                ...
                if wt_inv:
                    ...
                    for (
                        partial_values
                    ) in st.invoice_id._get_all_reconciled_invoice_partials():
                        payment_move = partial_values["aml"].move_id
                        payment_moves_to_amount.setdefault(
                            payment_move,
                            payment_moves_to_amount.get(payment_move, 0)
                            + partial_values.get("amount"),
                        )
